...
def rightShift(key,plain_text):
  key = unicode2Text(key)
  temp = ""
  temp1 = 0
  for i in range(len(plain_text)):
    temp1 += ord(plain_text[i])
  
  for i in range(1, len(key)+1):
    temp += chr((ord(key[i-1]) + (temp1)%i)%i)
  return text2Unicode(temp)
